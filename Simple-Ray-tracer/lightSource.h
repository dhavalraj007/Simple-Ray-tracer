...
public:
	...
	color getColor(dpoint p, dvec3 normalAtp,hittableList worldObjects) const
	{
		...
		{
			if (object->hit(r, 0.001, infinity).first)
		...
		if (angle > glm::radians(90.f) or angle < glm::radians(-90.f))	//normal is pointing away from light
		{
